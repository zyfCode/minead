...
import com.sungan.ad.dao.OrderType;
...
//		bean.insert(weight0);
	...
	public AdPager<AdHourWeightVo> queryPager(AdHourWeight condition, int pageIndex, int rows) {
		AdPager<AdHourWeight> queryPage = adHourWeightDAO.queryPageInOrder(condition, pageIndex, rows,OrderType.ASC,AdHourWeightVo.COLUMN_HOUR);
		...
		if(parseToVoList!=null){ 
			for(AdHourWeightVo adVo:parseToVoList){
