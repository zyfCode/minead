 ...
 * @version V1.1
 */
public class TestDaillyTaskAppDR implements TaskApp{
	private static final String version = "v1.0";
	private static final Log  log  = LogFactory.getLog(TestDaillyTaskAppDR.class);
		...
		excutor.scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
				excutor.execute(new Runnable() {
					...
					public void run() {
						try {
							log.debug(actuallCount+"/"+TestDaillyTaskAppDR.this.add(1)+"任务开始....");
							TestDaillyTaskAppDR.this.doExcute();
							log.debug(actuallCount+"/"+TestDaillyTaskAppDR.this.add(0)+"===任务完成...");
						} catch (Exception e) {
							log.error("任务异常", e);
						}
					}
